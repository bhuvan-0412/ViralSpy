...
export const isDemoModeActive = () => {
  ...
  const guestUser = localStorage.getItem('viralspy_guest_user');
...
export async function getCurrentUser() {
  ...
  if (isDemoModeActive()) {
    const guest = localStorage.getItem('viralspy_guest_user');
    if (guest) {
...
export async function signInAsGuest() {
  ...
  localStorage.setItem('viralspy_guest_user', JSON.stringify(guestUser));
  return guestUser;
}
...
export async function signOutUser() {
  ...
  localStorage.removeItem('viralspy_guest_user');
  localStorage.removeItem('viralspy_demo_niches');
...
export async function getUserNiches() {
  ...
  if (isDemoModeActive()) {
    const savedNiches = localStorage.getItem('viralspy_demo_niches');
...
export async function saveUserNiches(niches) {
  ...
  localStorage.setItem('viralspy_demo_niches', JSON.stringify(niches));
