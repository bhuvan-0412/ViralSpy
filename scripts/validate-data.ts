...
const VALID_FORMATS = ['TALKING_HEAD', 'POV', 'DUET', 'TUTORIAL', 'STORYTIME', 'TRANSITION']
...
async function main() {
  ...
  lines.forEach((line, i) => {
    try {
      ...
      const missing = REQUIRED_FIELDS.filter((f) => !output[f])
      if (missing.length > 0) {
      ...
      if (hookWords > 10) {
        issues.push(`Line ${i + 1}: Hook too long (${hookWords} words): "${output.hook}"`)
      }

      // Check format is valid
      if (!VALID_FORMATS.includes(output.format)) {
        issues.push(`Line ${i + 1}: Invalid format: ${output.format}`)
        invalid++
        return
      }

      // Check angles count
      if (!Array.isArray(output.angles) || output.angles.length !== 3) {
        issues.push(`Line ${i + 1}: Expected 3 angles, got ${output.angles?.length}`)
        invalid++
        return
      }
      ...
      if (!Array.isArray(output.hashtags) || output.hashtags.length < 3) {
        issues.push(`Line ${i + 1}: Too few hashtags: ${output.hashtags?.length}`)
        invalid++
        return
      }
  ...
  console.log(`📊 Quality score: ${Math.round((valid / lines.length) * 100)}%`)
