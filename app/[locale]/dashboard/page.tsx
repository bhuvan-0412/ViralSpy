...
export default function DashboardPage() {
  ...
  const getBriefsPath = () => (locale === 'en' ? '/briefs' : `/${locale}/briefs`)
    ...
    <div className="min-h-screen bg-[#F7F5F2] text-[#1A1A1A] flex flex-col justify-between font-sans">
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          ...
          <nav className="hidden md:flex items-center space-x-8 text-sm font-semibold text-gray-655">
            <button
              ...
              className="text-gray-500 hover:text-[#FF6B4A] transition-colors"
            >
              {t('title')}
            </button>
            <button
            ...
            <button
              onClick={() => router.push(getLocalizedPath('/feedback'))}
              className="hover:text-[#FF6B4A] transition-colors flex items-center gap-1.5"
            >
      ...
      <main className="flex-grow max-w-7xl mx-auto w-full py-8 px-4 sm:px-6 space-y-8">
        ...
        {(() => {
          ...
          const trendsDetectedToday = trends.filter(
            (t) => new Date(t.detected_at) > oneDayAgo
          ).length
          const explodingRightNow = trends.filter((t) => t.momentum_status === 'EXPLODING').length
          const risingAndExploding = trends.filter(
            (t) => t.momentum_status === 'RISING' || t.momentum_status === 'EXPLODING'
          )
          const avgVelocity = risingAndExploding.length
            ? Math.round(
                risingAndExploding.reduce((acc, t) => acc + t.velocity_score, 0) /
                  risingAndExploding.length
              )
            : 0
          ...
          return (
            <section className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 animate-fade-up delay-75">
              ...
              <div className="bg-white p-5 border border-gray-200 rounded-2xl shadow-card flex flex-col justify-between h-32 relative overflow-hidden transition-all duration-200 hover:shadow-md">
                ...
                <div className="mt-2">
                  <span className="text-3xl font-black text-[#1A1A1A] tracking-tight">
                    {formatIndianNumber(trendsDetectedToday)}
                  </span>
                </div>
              ...
              <div className="bg-white p-5 border border-gray-200 rounded-2xl shadow-card flex flex-col justify-between h-32 relative overflow-hidden transition-all duration-200 hover:shadow-md">
                ...
                <div className="mt-2">
                  <span className="text-3xl font-black text-red-500 tracking-tight">
                    {formatIndianNumber(explodingRightNow)}
                  </span>
                </div>
                <div className="h-8 w-full mt-2">
              ...
              <div className="bg-white p-5 border border-gray-200 rounded-2xl shadow-card flex flex-col justify-between h-32 relative overflow-hidden transition-all duration-200 hover:shadow-md">
                ...
                <div className="mt-2">
                  <span className="text-3xl font-black text-[#7F77DD] tracking-tight">
                    {formatIndianNumber(avgVelocity)}%
                  </span>
                </div>
                <div className="h-8 w-full mt-2">
              ...
              <div className="bg-white p-5 border border-gray-200 rounded-2xl shadow-card flex flex-col justify-between h-32 relative overflow-hidden transition-all duration-200 hover:shadow-md">
                ...
                <div className="mt-2">
                  <span className="text-3xl font-black text-[#1D9E75] tracking-tight">
                    {formatIndianNumber(briefsCount)}
                  </span>
                </div>
                <div className="h-8 w-full mt-2">
          ...
          {loadingTrends ? (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {[...Array(4)].map((_, i) => (
                <div
                  key={i}
                  className="bg-white border border-gray-200 rounded-2xl p-5 space-y-4 animate-pulse shadow-card"
                >
