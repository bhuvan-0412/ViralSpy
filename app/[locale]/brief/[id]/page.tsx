...
export default function BriefPage() {
  ...
  useEffect(() => {
    ...
    const msgInterval = setInterval(() => {
      setBriefMessageIndex((prev) => (prev < briefMessages.length - 1 ? prev + 1 : prev))
    }, 4000)

    // Progress bar fills to 90% over 28 seconds
    // then waits for actual completion
    const progressInterval = setInterval(() => {
      setBriefProgress((prev) => (prev < 90 ? prev + 90 / 28 : prev))
    }, 1000)
  ...
  const handleRegenerate = async () => {
    ...
    try {
      ...
      if (data.success && data.data) {
        setBrief(data.data)
        const localizedPath =
          locale === 'en' ? `/brief/${data.data.id}` : `/${locale}/brief/${data.data.id}`
    ...
    <div className="min-h-screen bg-[#F7F5F2] text-[#1A1A1A] flex flex-col justify-between font-sans relative overflow-hidden">
      {isLoading ? (
        <div className="min-h-screen bg-[#F7F5F2] flex flex-col items-center justify-center px-4 w-full z-50">
          ...
          {/* Time estimate */}
          <p className="text-xs text-gray-400 text-center">Usually 5–30 seconds with local AI</p>
              ...
              <main className="flex-grow max-w-[720px] mx-auto w-full py-6 px-4 sm:px-0">
                ...
                {(() => {
                  const briefAge = brief.created_at
                    ? Math.floor(
                        (Date.now() - new Date(brief.created_at).getTime()) / (1000 * 60 * 60)
                      )
                    : 0
                  const isOutdated = briefAge >= 6
                  return (
                    <div
                      className={`flex items-center justify-between px-4 py-2 rounded-xl text-sm mb-4 ${
                        isOutdated
