export async function POST(request: Request) {
  const { key, provider, baseUrl, model } = await request.json()
  ...
  try {
    if (provider === 'gemini') {
      ...
      return Response.json({ valid: true, message: 'Gemini key valid ✓' })
    }
    ...
    return Response.json({ valid: true, message: 'API key valid ✓' })
  } catch (e: any) {
    return Response.json({ valid: false, message: e.message })
  }
}
