...
export default function TrendCard({
  ...
  const handleGenerateBrief = async () => {
    ...
    try {
      ...
      if (provider === 'ollama') {
        ...
        if (result.data?.id) {
          const localizedPath =
            locale === 'en' ? `/brief/${result.data.id}` : `/${locale}/brief/${result.data.id}`
  ...
  const sparklineData = Array.isArray(trend.raw_data?.sparkline)
    ...
    : [
        trend.velocity_score * 0.4,
        trend.velocity_score * 0.6,
        trend.velocity_score * 0.8,
        trend.velocity_score,
      ]
  ...
  const containerClasses = `bg-white border rounded-2xl p-5 hover:border-gray-300 transition-all flex flex-col justify-between h-full shadow-card ${
    isExploding ? 'border-l-4 border-l-[#FF6B4A] border-gray-250' : 'border-gray-200'
  }`
