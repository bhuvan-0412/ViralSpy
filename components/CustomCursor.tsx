    ...
    const animate = () => {
      ...
      if (trail.current.length > 2) {
        ...
        for (let i = 1; i < trail.current.length - 1; i++) {
          const mx = (trail.current[i].x + 
            trail.current[i + 1].x) / 2;
          const my = (trail.current[i].y + 
            trail.current[i + 1].y) / 2;
          ctx.quadraticCurveTo(
            trail.current[i].x, trail.current[i].y, 
            mx, my
          );
        }
        ...
        grad3.addColorStop(0.3, 
          `rgba(255, 255, 255, 0.15)`);
        grad3.addColorStop(0.7, 
          `rgba(255, 255, 255, 0.6)`);
        grad3.addColorStop(1, 
      ...
      orbPos.current.x += 
        (mousePos.current.x - orbPos.current.x) * 0.14;
      orbPos.current.y += 
        (mousePos.current.y - orbPos.current.y) * 0.14;

      if (orbRef.current) {
        orbRef.current.style.left = 
          `${orbPos.current.x}px`;
        orbRef.current.style.top = 
          `${orbPos.current.y}px`;
      }
